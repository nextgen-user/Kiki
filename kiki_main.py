...
keep_listening_tool = {
    "name": "keep_listening_for_follow_up",
    "description": '''

Purpose: After your current response, remain in active listening mode, ready to immediately process a follow-up question or statement from the user without requiring them to use the hotword again.

When to Use: Employ this command when you have just asked a question, requested clarification, or when you expect the user to naturally continue the conversation immediately after your response.

Why: This ensures a smoother, more natural conversational flow, especially during clarification or question-and-answer sequences.''',

    # "description": "Instructs the assistant to remain in active listening mode after its current response, anticipating a follow-up question or statement from the user. Use this when you ask a question or expect the user to continue the conversation without needing to say the hotword again.Always use this tool once or twice for following up and not more than that.After that, the assistant will stop listening and wait for the hotword to be said again.",
    "parameters": {
        "type": "object",
        "properties": {
            "reason": {
                "type": "string",
                "description": "Optional: A brief reason or prompt the AI is using to solicit the follow-up (e.g., 'Asking for more details', 'Clarification needed')."
            }
        },
        "required": []
    }
...
class AudioLoop:
    def __init__(self, video_mode=DEFAULT_MODE,
                 ...
                 porcupine_input_device_index=None):
        ...
        self.porcupine_input_device_index = porcupine_input_device_index
    ...
    async def _init_porcupine_resources(self):
        ...
        if self.porcupine is None:
            ...
            try:
                ...
                print(f"Failed to initialize Porcupine instance: {e}. Please check AccessKey and keyword paths.")
                self.porcupine = None
                return False
        ...
        if self.porcupine and (self.porcupine_stream is None or not self.porcupine_stream.is_active()):
            ...
            m.save()
            try:
                device_index_to_use = self.porcupine_input_device_index
                if device_index_to_use is None:
                     ...
                     print(f"Porcupine input device index not specified, using default: {device_index_to_use} ({default_device_info['name']})")
                else:
                    try:
                        dev_info = await asyncio.to_thread(pya.get_device_info_by_index, device_index_to_use)
                        print(f"Using specified Porcupine input device index: {device_index_to_use} ({dev_info['name']})")
                    except OSError:
                        old_idx = device_index_to_use
                        default_device_info = await asyncio.to_thread(pya.get_default_input_device_info)
                        device_index_to_use = default_device_info['index']
                        print(f"Warning: Specified Porcupine input device index {old_idx} not found. Using default: {device_index_to_use} ({default_device_info['name']}).")
    ...
    async def _handle_tool_call(self, tool_call) -> list[types.FunctionResponse]:
        function_responses = []
        print(f"\n  Handling tool call with {len(tool_call.function_calls)} function(s):")
        for fc in tool_call.function_calls:
            print(f"    Function Call: Name: {fc.name}, Args: {fc.args}")
            status = "success"
            result_message = f"Tool {fc.name} executed."

            try:
                if fc.name == "turn_on_the_lights":
                    print("      ACTION: Simulating turning lights ON.")
                    result_message = "Lights have been turned on."
                elif fc.name == "turn_off_the_lights":
                    print("      ACTION: Simulating turning lights OFF.")
                    result_message = "Lights have been turned off."
                elif fc.name == "keep_listening_for_follow_up":
                    print("      ACTION: AI requested to keep listening for a follow-up.")
                    reason = fc.args.get("reason", "No specific reason provided by AI.")
                    result_message = f"Instruction received: will keep listening for user's follow-up. Reason: {reason}"
                else:
                    print(f"      WARNING: Unknown tool function: {fc.name}")
                    result_message = f"Tool function {fc.name} is not implemented by the client."
                    status = "error"
            except Exception as e:
                print(f"      ERROR executing tool {fc.name}: {e}")
                result_message = f"Client-side error executing tool {fc.name}: {str(e)}"
                status = "error"
                traceback.print_exc()

            function_responses.append(
                types.FunctionResponse(
                    # Assuming fc.id is available from the LiveConnect SDK for FunctionCall
                    # If not, this might need adjustment based on SDK specifics for correlating responses
                    id=fc.id if hasattr(fc, 'id') else fc.name, # Fallback to name if id isn't present, though SDK should provide it.
                    name=fc.name,
                    response={"status": status, "message": result_message}
                )
            )
            ...
            while True:
                ...
                try:
                    ...
                    async for response_chunk in turn:
                        ...
                        # elif response_chunk.tool_call:

                        #     function_responses = await self._handle_tool_call(response_chunk.tool_call)
                        #     if not "keep_listening_for_follow_up" in str(function_responses) :# Make sure the user follows up before responding 
                        #         await self.session.send_tool_response(function_responses=function_responses)
                        #         follow_up=False
                        #     else:
                        #         print("-"*100+"\n    AI requested to keep listening for follow-up. No tool response sent."+"-"*100)
                        #         follow_up=True
                        #         # time.sleep(5)
                        #     print("    Tool response sent. Awaiting model's follow-up.")
...
def initialize_dependencies():
    ...
    try:
        pya = pyaudio.PyAudio()
        # You can list devices here for debugging if needed
        # print("Available audio input devices:")
        # for i in range(pya.get_device_count()):
        #     info = pya.get_device_info_by_index(i)
        #     if info.get('maxInputChannels') > 0:
        #         print(f"  Index {info['index']}: {info['name']} (Input Channels: {info['maxInputChannels']})")
    except Exception as e:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--pv_access_key", type=str, default=os.getenv("PVPORCUPINE_ACCESS_KEY"),
        help="Picovoice Porcupine AccessKey. Can also be set via PVPORCUPINE_ACCESS_KEY env var."
    )
    parser.add_argument(
        "--pv_keywords", nargs='+',
        help="Paths to Porcupine .ppn keyword files (space-separated) or built-in keyword names (e.g., 'porcupine')."
    )
    parser.add_argument(
        "--pv_input_device_index", type=int, default=None,
        help="Input device index for Porcupine. If not set, uses PyAudio default input device."
    )

    args = parser.parse_args()

    if not args.pv_access_key:
        print("Error: Picovoice Porcupine AccessKey is required. "
              "Set PVPORCUPINE_ACCESS_KEY environment variable or use --pv_access_key.")
        sys.exit(1)

    # Handle default/built-in keywords for Porcupine
    if not args.pv_keywords:
        print("No Porcupine keywords provided via --pv_keywords.")
        # Attempt to use a common built-in keyword if available
        try:
            # Check for built-in keywords in pvporcupine. Some versions store them in KEYWORD_PATHS
            if hasattr(pvporcupine, 'KEYWORD_PATHS') and pvporcupine.KEYWORD_PATHS:
                # Try a common one like 'porcupine' or 'hey siri', 'alexa' etc., if they exist
                # For this example, let's default to 'porcupine' if it exists.
                # You might want to make this configurable or list available built-ins.
                ...
                if keywords_dir and os.path.isdir(keywords_dir):
                    ...
                    if available_ppns:
                        # Try to find a "Hey Kiki" or "Porcupine" like keyword
                        # For simplicity, just take the first one found.
                        # You should customize this to your needs or provide a specific default .ppn file.
                        default_kw = os.path.join(keywords_dir, available_ppns[0])
                        args.pv_keywords = [default_kw]
                        print(f"Using a found built-in Porcupine keyword: {args.pv_keywords[0]}")
                    else:
                        print("Could not find any .ppn files in Porcupine's built-in keyword directory.")
                        raise ValueError("No default keywords found.")

                else:
                    raise ValueError("No built-in keywords accessible and none provided.")

        except Exception as e_kw:
            print(f"Error setting up default/built-in Porcupine keywords: {e_kw}")
            print("Please provide valid keyword paths using --pv_keywords (e.g., path/to/your.ppn)")
    ...
    if args.pv_keywords:
        for kw_arg in args.pv_keywords:
            if os.path.exists(kw_arg): # It's a path
                validated_keywords.append(kw_arg)
            else: # Assume it's a built-in keyword name
                # pvporcupine.create handles built-in names directly
                print(f"Assuming '{kw_arg}' is a built-in Porcupine keyword name.")
                validated_keywords.append(kw_arg) 
        args.pv_keywords = validated_keywords
    ...
    audio_loop_instance = AudioLoop(
        ...
        porcupine_access_key=args.pv_access_key,
        porcupine_keyword_paths=args.pv_keywords, # This should now be a list of paths or names
        porcupine_input_device_index=args.pv_input_device_index
    )
